...
**Estado atual (2026-08-20):** o editor mantém nome/dose separados dos horários estruturados e há uma tabela reutilizável orientada ao cuidador com medicamento, dose, via, marcação visual/textual dos momentos, uso contínuo e observações, incluindo layout responsivo e impressão. Texto e tabela compartilham um único `MedicationPlanViewModel` validado no domínio. O histórico de status usa apenas eventos explícitos `ACTIVE` / `SUSPENDED` / `FINISHED`, persistidos em `MedicationStatusEvent` com chaves compostas paciente/medicamento/consulta e sem backfill do estado atual. A escrita prospectiva deriva paciente da consulta, bloqueia consulta finalizada ou anterior, verifica coerência do último evento e usa compare-and-set em transação `Serializable`. A interface clínica já exibe o status derivado do servidor e exige confirmação explícita de revisão antes de registrar alterações; mudar a seleção invalida a confirmação local. A fronteira HTTP `POST /api/consultations/[id]/medications/status` aceita exclusivamente `medicationId` + `newStatus`; consulta/paciente/status anterior permanecem server-owned e campos extras são rejeitados. O contrato de `MEDICATION_PLAN` é fail-closed: `UNKNOWN` e `current-record-only` bloqueiam geração, somente `ACTIVE` entra no plano do cuidador e `SUSPENDED`/`FINISHED` ficam apenas na rastreabilidade técnica. A integração com `DocumentSnapshot` está em validação no PR #57, que persiste o plano versionado usando identidade consulta → paciente derivada no servidor e sanitiza falhas inesperadas do endpoint. O limite conhecido é que leitura do workspace e persistência do snapshot ainda são operações server-side sequenciais, não uma transação ponta a ponta.
