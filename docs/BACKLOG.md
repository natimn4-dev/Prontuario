...
**Estado atual (2026-08-19):** o editor mantém nome/dose separados dos horários estruturados e há uma tabela reutilizável orientada ao cuidador com medicamento, dose, via, marcação visual/textual dos momentos, uso contínuo e observações, incluindo layout responsivo e impressão. Texto e tabela compartilham um único `MedicationPlanViewModel` validado no domínio. O histórico de status usa apenas eventos explícitos `ACTIVE` / `SUSPENDED` / `FINISHED`, persistidos em `MedicationStatusEvent` com chaves compostas paciente/medicamento/consulta e sem backfill do estado atual. Existe agora uma operação interna autenticada e transacional para registrar mudanças prospectivas: deriva paciente da consulta, bloqueia consulta finalizada ou anterior, exige coerência entre o último evento explícito e `Medication.status`, usa compare-and-set para detectar concorrência e grava evento + estado atual + auditoria na mesma transação `Serializable`. Ainda falta expor essa operação por uma fronteira HTTP/UI estrita e, depois, integrar a projeção histórica revisada aos snapshots reais de `MEDICATION_PLAN`.
