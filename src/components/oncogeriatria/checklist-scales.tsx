...
export function CargChecklistForm({ patientId, episodeId, checkpointId, initialAgeYears, initialBiologicalSex, initialAnswers, initialProvenance, initialSavedAt, initialSavedBy, canFinalize = true, finalizationMessage, readOnly = false }: { patientId: string; episodeId: string; checkpointId: string; readOnly?: boolean; initialAgeYears?: number; initialBiologicalSex?: CargBiologicalSex; initialAnswers?: InitialAnswers; initialProvenance?: Record<string, unknown> | null; initialSavedAt?: string | null; initialSavedBy?: string | null; canFinalize?: boolean; finalizationMessage?: string }) {
  ...
  return <form className={styles.scaleForm} onSubmit={submit}>
    {readOnly ? <p role="status" className={styles.locked}>Consulta finalizada: registro disponível apenas para leitura.</p> : null}
    ...
    {!canFinalize ? <div className={styles.accessNotice} role="status"><strong>CARG disponível para preenchimento.</strong><p>Complete os 11 fatores e salve o rascunho nesta mesma tela. O registro definitivo será habilitado quando esta avaliação estiver vinculada a uma consulta clínica.</p></div> : null}
    ...
    <div className={styles.actions}><span>Faixas preservadas: 0–5 baixo; 6–9 intermediário; 10–23 alto. Máximo teórico 23; faixa observada original até 19.</span><div><button type="button" disabled={saving} onClick={() => void saveDraft()}>{saving ? "Salvando…" : "Salvar rascunho"}</button> <button type="submit" disabled={saving || !preview || !canFinalize}>{saving ? "Salvando…" : canFinalize ? "Calcular e registrar CARG" : "Vincule uma consulta para registrar"}</button></div></div>
