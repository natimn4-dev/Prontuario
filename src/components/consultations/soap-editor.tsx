...
function renderSoap(draft: Draft, problems: Problem[]): string {
  ...
  const lines = [
    ...
    "Medicações em uso: consultar a reconciliação medicamentosa desta consulta.",
    "",
    "A — AVALIAÇÃO",
  ];
...
export function SoapEditor({ consultationId }: { consultationId: string }) {
  ...
  async function load() {
    ...
    try {
      ...
      setFeedback({ kind: "error", text: error instanceof Error ? error.message : "Não foi possível carregar o SOAP." });
    } finally {
      setLoading(false);
    }
  }
  ...
  function setField<K extends Exclude<keyof Draft, "planTextByProblem">>(key: K, value: Draft[K]) {
  ...
  async function save() {
    ...
    try {
      ...
      setDraft(draftFromView(body));
      setDirty(false);
      setFeedback({ kind: "success", text: "SOAP salvo nesta consulta." });
    } catch (error) {
      setFeedback({ kind: "error", text: error instanceof Error ? error.message : "Não foi possível salvar o SOAP." });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function copySoap() {
    ...
    try {
      await navigator.clipboard.writeText(renderSoap(draft, view.problems));
      setFeedback({ kind: "success", text: "SOAP copiado para a área de transferência." });
    } catch {
      setFeedback({ kind: "error", text: "Não foi possível copiar o SOAP neste navegador." });
    }
  }

  if (loading) return <section className={styles.card}><p>Carregando SOAP…</p></section>;
  if (!view || !draft) {
    return <section className={styles.card}><p role="alert">{feedback?.text ?? "SOAP indisponível."}</p></section>;
  }
    ...
    <section className={styles.card} aria-labelledby="soap-editor-title">
      <div className={styles.heading}>
        <div>
          ...
          <h2 id="soap-editor-title">Evolução SOAP</h2>
          <p className={styles.muted}>Salve antes de copiar. Avaliação usa somente problemas confirmados do histórico desta paciente.</p>
        </div>
        <div className={styles.actions}>
          <button type="button" onClick={save} disabled={!dirty || saving || finalized}>
            {saving ? "Salvando…" : "Salvar SOAP"}
          </button>
          <button type="button" className={styles.secondary} onClick={copySoap} disabled={dirty}>
      ...
      <div className={styles.soapGrid}>
        ...
        <section className={styles.soapSection}>
          ...
          <p className={styles.muted}>As medicações são derivadas da reconciliação medicamentosa e não são duplicadas neste JSON.</p>
        </section>
        ...
        <section className={styles.soapSection}>
          <h3>P — Plano por problema</h3>
          {activeProblems.length === 0 ? <p className={styles.muted}>Cadastre/confirme problemas para vincular condutas.</p> : activeProblems.map((problem) => (
            <label key={problem.id} className={styles.planField}>
              {problem.title}
              <span>Uma ação por linha. Nada é aplicado automaticamente.</span>
              <textarea
                value={draft.planTextByProblem[problem.id] ?? ""}
                disabled={finalized}
                onChange={(event) => setProblemPlan(problem.id, event.target.value)}
                rows={3}
              />
            </label>
          ))}
