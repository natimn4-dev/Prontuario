...
function RegimenFields({ form, setForm, includeName }: { form: Form; setForm: (value: Form) => void; includeName: boolean }) {
  function toggle(moment: MedicationMoment) {
    setForm({ ...form, moments: form.moments.includes(moment) ? form.moments.filter((item) => item !== moment) : [...form.moments, moment] });
  }
...
export function MedicationWorkspace({ consultationId, patientName }: { consultationId: string; patientName: string }) {
  ...
  function notify(next: View) {
    setView(next); setFeedback(null);
  ...
  async function createMedication() {
    ...
    try {
      const next = await post({ action: "create", name: form.name, presentation: form.presentation || undefined, doseInstruction: form.doseInstruction || undefined, route: form.route || undefined, moments: form.moments, continuous: form.continuous, instructions: form.instructions || undefined });
  ...
  function startRegimen(item: Item) {
    setEditingId(item.medicationId);
    setEditingForm({ name: item.name, presentation: item.presentation ?? "", doseInstruction: item.doseInstruction ?? "", route: item.route ?? "", moments: [...item.moments], continuous: item.continuous, instructions: item.instructions ?? "" });
  }
  ...
  async function saveRegimen() {
    ...
    setSaving(true); setFeedback(null);
    try {
      const next = await post({ action: "regimen", medicationId: editingId, doseInstruction: editingForm.doseInstruction || undefined, route: editingForm.route || undefined, moments: editingForm.moments, continuous: editingForm.continuous, instructions: editingForm.instructions || undefined });
  ...
  async function changeStatus(item: Item) {
    ...
    if (selected === item.status) return;
    setSaving(true); setFeedback(null);
    try {
  ...
  return <section className={styles.card} aria-labelledby="medication-workspace-title">
    ...
    {!view.isLatestConsultation ? <p className={styles.notice}>Consulta histórica: alterações retrospectivas estão bloqueadas.</p> : null}
