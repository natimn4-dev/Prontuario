...
export default async function OncogeriatricPostTreatmentPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ episode?: string }> }) {
  ...
  const workspace = await loadEpisodeWorkspace(patientId, episode.id, "post-treatment");
  ...
  const checkpointOptions = workspace.checkpoints.map((item) => ({ id: item.id, label: `${formatClinicalDate(item.occurredAt)} · ${oncogeriatricCheckpointTypeLabel(item.type)}` }));
  ...
  return (
    <main className="shell">
      ...
      <OncogeriatricClinicalContinuity patientId={patientId} consultation={workingConsultation} episodeId={episode.id} returnStage="pos-tratamento" />
      ...
      <section className="two-columns">
        ...
        <article className="panel"><h2>Registrar recuperação por domínio</h2><RecoveryForm patientId={patientId} episodeId={episode.id} consultations={consultationOptions} checkpoints={checkpointOptions} /><p className="muted">Use quando houver avaliação clínica. A ausência de preenchimento não é interpretada como recuperação.</p></article>
      </section>
      ...
      <section className="two-columns">
        ...
        <article className="panel">
          ...
          {workspace.recovery.length ? <ul className="clean-list">{workspace.recovery.map((item) => <li key={item.id}><strong>{oncogeriatricDomainLabel(item.domain)} · {oncogeriatricRecoveryStatusLabel(item.status)}</strong><span>{formatClinicalDate(item.assessedAt)}{item.consultationId ? " · consulta vinculada" : " · marco temporal sem consulta vinculada"}{item.notes ? ` · ${item.notes}` : ""}</span></li>)}</ul> : <p className="muted">Nenhum domínio de recuperação avaliado.</p>}
        </article>
      </section>
