...
export function DietaryAssessmentWorkspace({ consultationId }: { consultationId: string }) {
  ...
  return <div className={styles.workspace}>
    <header className={styles.header}><div><span className={styles.eyebrow}>Recordatório alimentar e apoio à decisão</span><h3>Avaliação alimentar</h3><p>O relato é autorreferido. Confirme alimento, quantidade, unidade e preparo antes de interpretar nutrientes.</p></div><div className={styles.headerMeta}><strong>{totalItems} alimento{totalItems === 1 ? "" : "s"}</strong><span>{data?.clinicalContext.weightKg ? `Peso disponível: ${format(data.clinicalContext.weightKg, 1)} kg` : "Sem peso disponível"}</span></div></header>
    ...
    <section className={styles.card} aria-labelledby="dietary-entry-title">
      ...
      <div className={styles.warning}><strong>Como registrar porções</strong><p>Ovo: informe quantas unidades foram consumidas. Exemplo: 1 ovo = 1 unidade média; 2 ovos = 2 unidades médias.</p><p>Carne, frango ou peixe: informe o número de pedaços ou o peso. “1 palmo” é apenas uma estimativa visual e deve ser confirmado quando possível.</p><p>Exemplos: 1 filé de peixe = confirmar peso · 1 pedaço de frango = informar corte · 1 bife = confirmar peso · palma da mão = estimativa, não medida exata.</p></div>
