...
export default async function ConsultationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const consultation = await prisma.consultation.findUnique({
    where: { id },
    select: {
      id: true, type: true, status: true, occurredAt: true,
      patient: { select: { id: true, fullName: true, birthDate: true, needsIdentityReview: true } },
    },
  });
    ...
    <main className="shell consultation-shell">
      <header className="hero compact-hero clinical-hero">
        ...
        <dl className={styles.contextGrid} aria-label="Identificação da consulta atual">
          <div><dt>Data de nascimento</dt><dd>{context.patientBirthDateLabel}</dd></div>
          <div><dt>Tipo de consulta</dt><dd>{context.consultationTypeLabel}</dd></div>
          <div><dt>Data da consulta</dt><dd>{context.consultationDateLabel}</dd></div>
        </dl>
        ...
        <p className={styles.intro}>Confirme a lista longitudinal de problemas, registre a evolução SOAP e complete medicamentos e avaliações geriátricas antes da revisão final.</p>
        <ol className="workflow-steps" aria-label="Etapas da consulta"><li><span>1</span>Problemas</li><li><span>2</span>SOAP</li><li><span>3</span>Avaliar</li><li><span>4</span>Compartilhar</li></ol>
      </header>
