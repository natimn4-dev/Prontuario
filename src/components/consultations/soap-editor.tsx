...
function renderSoap(draft: Draft, problems: Problem[]): string {
  ...
  const lines = ["S — SUBJETIVO", valueOrMissing(draft.subjective), "", "O — OBJETIVO", `Exame físico: ${valueOrMissing(draft.physicalExam)}`, `Sinais vitais: ${valueOrMissing(draft.vitalSigns)}`, `Antropometria: ${valueOrMissing(draft.anthropometry)}`, "Medicações em uso: consultar a reconciliação medicamentosa desta consulta.", "", "A — AVALIAÇÃO"];
  ...
  if (active.length === 0) lines.push("sem dados registrados");
    ...
    if (actions.length === 0) lines.push("- sem dados registrados"); else actions.forEach((action) => lines.push(`- ${action}`));
  });
  return lines.join("\n");
}
...
export function SoapEditor({ consultationId }: { consultationId: string }) {
  ...
  useEffect(() => {
    function onProblemsChanged(event: Event) {
      ...
      if (dirty) {
        setFeedback({ kind: "error", text: "A lista de problemas mudou enquanto há alterações não salvas no SOAP. Salve ou recarregue antes de continuar o plano." });
        return;
      }
      void load();
    }
    window.addEventListener("clinical-problems-changed", onProblemsChanged);
    return () => window.removeEventListener("clinical-problems-changed", onProblemsChanged);
  ...
  async function copySoap() {
    ...
    try { await navigator.clipboard.writeText(renderSoap(draft, view.problems)); setFeedback({ kind: "success", text: "SOAP copiado para a área de transferência." }); }
  ...
  return <section className={styles.card} aria-labelledby="soap-editor-title">
    <div className={styles.heading}><div><p className="eyebrow">Prontuário</p><h2 id="soap-editor-title">Evolução SOAP</h2><p className={styles.muted}>Salve antes de copiar. Avaliação usa somente problemas confirmados do histórico desta paciente.</p></div><div className={styles.actions}><button type="button" onClick={save} disabled={!dirty || saving || finalized}>{saving ? "Salvando…" : "Salvar SOAP"}</button><button type="button" className={styles.secondary} onClick={copySoap} disabled={dirty}>Copiar para prontuário</button></div></div>
