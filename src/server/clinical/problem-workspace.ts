...
async function context(tx: Prisma.TransactionClient, consultationId: string) {
  ...
  if (!consultation) {
    throw new ProblemWorkspaceError("CONSULTATION_NOT_FOUND", "Consulta não encontrada.");
  }
...
export async function createProblem(command: CreateProblemCommand): Promise<ProblemWorkspaceView> {
  ...
  return prisma.$transaction(async (tx) => {
    const current = await context(tx, command.consultationId);
    assertEditable(current);

    const title = command.title.trim();
    if (!title) throw new Error("Título do problema é obrigatório.");
    ...
    const created = await tx.clinicalProblem.create({
      data: {
        ...
        originConsultationId: current.consultation.id,
        type: command.type,
        status: "ACTIVE",
        title,
        description,
    ...
    await tx.auditEvent.create({
      data: {
        ...
        outcome: "success",
        reasonCode: command.type === "GERIATRIC" ? "geriatric-problem" : "clinical-problem",
      },
    });
...
export async function changeProblemStatus(
  command: ChangeProblemStatusCommand,
): Promise<ProblemWorkspaceView> {
  const { user } = await requireAuthenticatedUser("consultation.write");
  return prisma.$transaction(async (tx) => {
    const current = await context(tx, command.consultationId);
    assertEditable(current);
    ...
    if (!projected) {
      throw new ProblemWorkspaceError("PROBLEM_NOT_FOUND", "Problema não encontrado nesta consulta.");
    }
    ...
    const updated = await tx.clinicalProblem.updateMany({
      ...
      data: {
        ...
        resolvedAt: command.newStatus === "RESOLVED" ? new Date() : null,
      },
    });
    if (updated.count !== 1) {
      throw new ProblemWorkspaceError(
        "PROBLEM_CHANGED",
        "O problema foi alterado em outra sessão. Recarregue antes de tentar novamente.",
      );
    }
    ...
    await tx.auditEvent.create({
      data: {
        ...
        action: "problem.status.change",
        requestId: command.requestId,
        outcome: "success",
        reasonCode: `${projected.status.toLowerCase()}-to-${command.newStatus.toLowerCase()}`,
      },
    });
