            ...
            <select value={selected} onChange={(event) => setSelected(event.target.value as ComplementaryScoreScaleCode)} disabled={saving}>
              {groupedDefinitions.map(([dimension, items]) => (
                <optgroup key={dimension} label={dimensionLabel(dimension)}>
                  {items.map((item) => <option key={item.code} value={item.code}>{item.name.replace(" — registro rápido de pontuação", "").replace(" — registro de pontuação", "")}</option>)}
                </optgroup>
              ))}
            </select>
          </label>
            ...
            <div className={styles.workspace}>
              <header className={styles.workspaceHeader}>
                <div>
                  ...
                  <h3>{definition.name.replace(" — registro rápido de pontuação", "").replace(" — registro de pontuação", "")}</h3>
                </div>
