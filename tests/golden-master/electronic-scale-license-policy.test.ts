...
test("cada autorização precisa ser confirmada individualmente", () => {
  ...
  assert.equal(isElectronicScaleLicensed("meem_freitas", flags), false);
  const restrictions = unconfirmedElectronicScaleRestrictions(flags);
  assert.deepEqual(restrictions.map((item) => item.code), ["meem_freitas"]);
});

test("restrição informa o instrumento sem expor segredo ou credencial", () => {
  const flags = electronicScaleLicenseFlagsFromEnvironment({});
  const restriction = electronicScaleRestriction("moca_br_freitas", flags);
