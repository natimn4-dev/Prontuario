...
import type { ProblemWorkspaceView } from "../../domain/problem-workspace.ts";
...
type Operations = {
  ...
  createProblem(input: CreateCommand & { consultationId: string; requestId?: string }): Promise<ProblemWorkspaceView>;
  changeProblemStatus(input: StatusCommand & { consultationId: string; requestId?: string }): Promise<ProblemWorkspaceView>;
};

function asRecord(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new ProblemWorkspaceRequestError("Requisição inválida.");
  }
...
function assertOnlyKeys(record: Record<string, unknown>, allowed: readonly string[]): void {
  ...
  if (Object.keys(record).some((key) => !allow.has(key))) {
    throw new ProblemWorkspaceRequestError("A requisição contém campos não permitidos.");
  }
}

function safeId(value: unknown, label: string): string {
  if (typeof value !== "string" || !value.trim() || value.length > 191) {
    throw new ProblemWorkspaceRequestError(`${label} inválido.`);
  }
  return value.trim();
}
...
export function parseProblemWorkspaceCommand(body: unknown): ProblemWorkspaceCommand {
  ...
  if (record.action === "create") {
    ...
    if (typeof record.type !== "string" || !TYPES.has(record.type)) {
      throw new ProblemWorkspaceRequestError("Tipo de problema inválido.");
    }
    if (typeof record.title !== "string" || !record.title.trim() || record.title.length > MAX_TITLE) {
      throw new ProblemWorkspaceRequestError("Título do problema inválido.");
    }
    if (record.description !== undefined && (typeof record.description !== "string" || record.description.length > MAX_DESCRIPTION)) {
      throw new ProblemWorkspaceRequestError("Descrição do problema inválida.");
    }
    return {
      action: "create",
      type: record.type as ProblemType,
  ...
  if (record.action === "status") {
    ...
    if (typeof record.newStatus !== "string" || !STATUSES.has(record.newStatus)) {
      throw new ProblemWorkspaceRequestError("Status do problema inválido.");
    }
    return {
      action: "status",
      problemId: safeId(record.problemId, "Identificador do problema"),
      newStatus: record.newStatus as ProblemStatus,
    };
  }
...
function errorResponse(error: unknown): Response {
  if (error instanceof ProblemWorkspaceRequestError) {
    return json({ code: "INVALID_REQUEST", message: error.message }, 400);
  }
  if (error instanceof ProblemWorkspaceError) {
    return json({ code: error.code, message: error.message }, error.code === "CONSULTATION_NOT_FOUND" ? 404 : 409);
  }
...
export function problemWorkspaceHttpHandlers(operations: Operations) {
  return {
    GET: async (_request: Request, consultationId: string): Promise<Response> => {
      try {
        return json(await operations.getProblemWorkspace(consultationId), 200);
      } catch (error) {
    ...
    POST: async (request: Request, consultationId: string): Promise<Response> => {
      try {
        ...
        const view = command.action === "create"
          ? await operations.createProblem({ ...command, consultationId, requestId: operationalRequestId })
          : await operations.changeProblemStatus({ ...command, consultationId, requestId: operationalRequestId });
