...
**Estado atual (2026-08-19):** o editor mantém nome/dose separados dos horários estruturados e há uma tabela reutilizável orientada ao cuidador com medicamento, dose, via, marcação visual/textual dos momentos, uso contínuo e observações, incluindo layout responsivo e impressão. Texto e tabela compartilham um único `MedicationPlanViewModel` validado no domínio, com uma linha por medicamento e estado booleano independente para cada horário; o cabeçalho falha fechado se não houver paciente identificado ou se o nome tentar ocupar múltiplas linhas. O domínio possui corte temporal conservador de `MedicationRegimen` e projeção de status exclusivamente por eventos explícitos `ACTIVE` / `SUSPENDED` / `FINISHED`. O schema agora inclui `MedicationStatusEvent`, ligado por chaves compostas ao medicamento e à consulta do mesmo paciente; a migration cria a tabela vazia, sem backfill a partir de `Medication.status`, e testes MySQL bloqueiam cruzamento de pacientes. Ainda falta expor uma operação autenticada/transacional para registrar eventos prospectivos antes de ligar essa informação aos snapshots reais de `MEDICATION_PLAN`.
